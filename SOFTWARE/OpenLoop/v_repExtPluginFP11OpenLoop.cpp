...
VREP_DLLEXPORT void* v_repMessage(int message,int* auxiliaryData,void* customData,int* replyData)
{
	...
	{	// This message is sent each time the scene was rendered (well, shortly after) (very often)
		...
		if (simGetSimulationState() == sim_simulation_advancing_running){
			...
			w_l = (1 / (2 * wheel_radius))*(2  - axel_length);
			w_r = (1 / (2 * wheel_radius))*(2  + axel_length);
...
void linear_controller(){
	...
	if (gain_mod == 1){
		...
		k1 = 2*chi*a;
		k3 = k1;
		k2 = ((a*a)-(norm_wd*norm_wd))/(norm_vd*norm_vd);
	}
...
void non_linear_controller(){
	...
	simFloat u1 = -k1*error[0];
	simFloat u2 = -k2*norm_vd*sin(error[2])/error[2]*error[1]-k3*error[2];
...
void cartesian_reg()
{
	...
	w = k2*(atan2(currentConf[1], currentConf[0])  + pi- currentConf[2]);
...
void posture_reg()
{
	...
	if (norm < 0.03 && currentConf[2] < 0.087) { 
		stopSim();
	}
}
