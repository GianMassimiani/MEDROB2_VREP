...
simChar* finalConf = "";
simFloat currentConf[3], desiredConf[3],past_pose[3],real_pose[3], old_kin_pose[3];
bool dynamic = false;
...
VREP_DLLEXPORT void* v_repMessage(int message,int* auxiliaryData,void* customData,int* replyData)
{
	...
	{	// This message is sent each time the scene was rendered (well, shortly after) (very often)
		...
		if (simGetSimulationState() == sim_simulation_advancing_running){
			...
			float linear_saturation = 1;
			if(v > linear_saturation){
				v = linear_saturation;
			}
			if(v < -linear_saturation){
				v = -linear_saturation;
			...
			float angular_saturation = 12;
			if(w > angular_saturation){
				w = angular_saturation;
			}
			if(w < -angular_saturation){
				w = -angular_saturation;
			}
			...
			if(dynamic ==  true) {
				...
				simSetJointTargetVelocity(motorRight,w_r);
					...
					// set kin_point pose to be the old_kin_pose
					simFloat tmp1[3],tmp2[3];

					//simGetObjectPosition(kin_point,-1,tmp1);
					tmp1[0] = old_kin_pose[0];
					tmp1[1] = old_kin_pose[1];
					tmp1[2] = robot_height;
					simSetObjectPosition(kin_point, -1, tmp1);

					//simGetObjectOrientation(kin_point,-1,tmp2);
					tmp2[0] = 0.0;
					tmp2[1] = 0.0;
					tmp2[2] = old_kin_pose[2];
					simSetObjectOrientation(kin_point, -1, tmp2);

					// update the current pose
					currentConf[0] = old_kin_pose[0];
					currentConf[1] = old_kin_pose[1];
					currentConf[2] = old_kin_pose[2];

					// compute new old_kin_pose with integrating the current control velocities over the sampling time
					old_kin_pose[2] = currentConf[2] + w*time_step;  
					old_kin_pose[0] = currentConf[0] + v/w*(sin(old_kin_pose[2])-sin(currentConf[2]));
					old_kin_pose[1] = currentConf[1] - v/w*(cos(old_kin_pose[2])-cos(currentConf[2]));
			}
	...
	{ // Simulation is about to start
		...
		kin_point = simGetObjectHandle("Kin_point");
		...
		past_pose[0] = init[0];
		past_pose[1] = init[1];
		old_kin_pose[0] = init[0];
		old_kin_pose[1] = init[1];
		simGetObjectOrientation(ref_point, -1, init);
		initConf[2] = init[2];
		past_pose[2] = init[2];
		old_kin_pose[2] = init[2];
...
void dynamic_io_linearization(double t){
	...
	w = (-sin(currentConf[2])*u1+cos(currentConf[2])*u2)/v;
...
void posture_reg()
{
	...
	gamma = atan2(currentConf[1], currentConf[0]) - currentConf[2] + pi;
	delta = gamma + currentConf[2];

	//error bounding
	gamma = bound_angle_error(gamma);
	...
	v = k1*rho*cos(gamma);
	w = k2*gamma + (k1 / gamma)*(sin(gamma)*cos(gamma)*(gamma + k3*delta));
